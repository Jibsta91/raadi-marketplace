...
if ($ghPath) {
    Write-Host "Using GitHub CLI to create release..." -ForegroundColor Yellow
    
    try {
        gh release create $TAG_NAME --title $RELEASE_NAME --notes $RELEASE_BODY --target main --repo "$REPO_OWNER/$REPO_NAME"
        
        Write-Host "✅ Release created successfully!" -ForegroundColor Green
        Write-Host "🔗 View release: https://github.com/$REPO_OWNER/$REPO_NAME/releases/tag/$TAG_NAME" -ForegroundColor Blue
    }
    catch {
        Write-Host "❌ Failed to create release with GitHub CLI: $($_.Exception.Message)" -ForegroundColor Red
    }
}
else {
    ...
    Write-Host "4. Copy and paste the release notes from this script" -ForegroundColor White
    Write-Host ""
...
Write-Host ""
Write-Host "GitHub Actions Workflow Options:" -ForegroundColor Green
Write-Host "1. Manual Trigger: https://github.com/$REPO_OWNER/$REPO_NAME/actions/workflows/release.yml" -ForegroundColor Blue
Write-Host "2. Tag-based Trigger: Push a new tag to automatically trigger release" -ForegroundColor Blue
